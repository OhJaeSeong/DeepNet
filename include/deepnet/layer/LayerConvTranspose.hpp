...
/// 컨볼루션을 수행하는 레이어.
class LayerConvTranspose : public LayerWeighted {
    int _out_channel;
    int _height;
    int _width;
    int _stride;
  ...
  public:
    /// 생성자.
    LayerConvTranspose(const TensorGpu &x, Workspace &workspace, //
                       int out_channel, int height, int width,   //
                       int stride = 1, int padding = 0, bool bias = true, int group = 1);

    virtual const char *type(void) const override { return "ConvTranspose"; }
